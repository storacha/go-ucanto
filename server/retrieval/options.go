...
func WithServiceMethod[O ipld.Builder](can string, handleFunc ServiceMethod[O]) Option {
	return func(cfg *srvConfig) error {
		cfg.service[can] = func(ctx context.Context, input invocation.Invocation, invCtx server.InvocationContext) (transaction.Transaction[ipld.Builder, ipld.Builder], *RetrievalResponse, error) {
			tx, res, err := handleFunc(ctx, input, invCtx)
			if err != nil {
				return nil, nil, err
			}
